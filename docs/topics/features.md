...
To enable this feature, add `"enableEncryptionWithExternalKms": true` in `kubernetesConfig` and `objectId` in `servicePrincipalProfile`. Optionally, if you want to create Hardware Security Modules (HSM) type keys, then add `"keyVaultSku": "Premium"` to enable creation of Premium SKU Key Vault and RSA-HSM type key. Otherwise, by default `keyVaultSku` can be omitted and a Standard SKU Key Vault and a RSA type key will be created.
